...
<Grid items={items_arr} bind:items_arr cols={8} let:item rowHeight={100}>
  <div class="content" style="background: {item.static ? '#cce' : '#ccc'};border:1px solid black;">{item.id} max width:{item.max.w}</div>
</Grid>
...
<script>
  ...
  let items_arr = [gridHelp.item({ x: 0, y: 0, w: 2, h: 2, id: id(), max: { w: 2 } }), gridHelp.item({ x: 2, y: 0, w: 3, h: 2, id: id(), max: { w: 3 } }), gridHelp.item({ x: 5, y: 0, w: 3, h: 3, id: id(), max: { w: 4 } })];
</script>
