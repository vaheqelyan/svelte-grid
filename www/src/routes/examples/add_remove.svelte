...
<p><a href=examples/responsive>→ Responsive</a></p>

<p><a href=examples/static>← Static element</a></p>
...
<Grid {items} bind:items={items} {cols} let:item={item} rowHeight={100} gap={10}>
	<div class=content style="background: #ccc; border: 1px solid black;">
    <span on:click={remove.bind(null, item)} class=close>✕</span> <!-- close x -->
    {item.id}
  </div>
</Grid>